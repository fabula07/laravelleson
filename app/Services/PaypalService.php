...
class PaypalService implements Contract\PaypalServiceContract
{
    ...
    public function create(CreateOrderRequest $request)
    {
        try
        {
            ...
            $paypalOrder = $this->paypalClient->createOrder([
                'intent' => 'CAPTURE',
                'purchase_units' => [['amount' => ['currency_code' => config('paypal.currency'), 'value' => $total]]]
            ]);

            $request = array_merge($request->validated(), ['vendor_order_id' => $paypalOrder['id'], 'total' => $total]);
    ...
    public function capture(string $vendorOrderId)
    {
        try
        {
            ...
            $order = app(OrderRepositoryContract::class)->setTransaction($vendorOrderId, PaymentSystem::Paypal,

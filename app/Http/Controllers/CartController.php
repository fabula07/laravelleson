...
class CartController extends Controller
{
    ...
    {
        Cart::instance('cart')->add($product->id, $product->title, 1, $product->price)->associate(Product::class);
    ...
    {
        $data = $request->validate(['rowId' => ['required', 'string'], 'count' => ['required', 'numeric', 'min:1']]);
